...
- **前后端分离部署演示版（`server/` + `web/`）**——在原型基础上叠加真实鉴权、后端 API、数据库持久化与工作流闭环。前端直接复用 `demo/` 的页面渲染引擎（`web/public/engine/`），保证视觉与交互 1:1 一致。适合在此基础上进一步开发可交付客户验收的真实系统。
...
| 角色 | 用户名 | 密码 | 说明 |
| ------ | -------- | -------- | ------ |
| 管理员 | `admin` | `admin123` | 全部功能（演示登录，单角色） |
...
**自动重置（推荐）**：每次在登录页**单击"登 录"按钮**成功后，系统自动清空业务数据并按 demo 口径（demo-fixtures.json）重新填充——每场演示从干净态开始，无需手动操作。演示期间（派发、新建等）产生的数据只保留在当次会话，重新登录即消失。
...
> **定位**：本仓库交付的是「高保真原型 + 可运行演示基线」。它验证了业务模型、页面交互与核心闭环的可行性，但**不构成可直接交付政府客户上线的系统**。本章按能力域列出从演示基线到生产交付需要重点完善的功能，供立项和开发排期参考。
...
| 二期 | 全量数据接入与质量监控 + 通知协同 + 运维体系 | 3.1 + 3.5 + 3.7 + 3.8 |
| 三期 | AI 真实化 + 企业侧门户 | 3.3 + 3.4（门户部分） |
